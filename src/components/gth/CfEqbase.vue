          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Equivalencias Importación</v-list-item-title
              >
            </v-list-item-content>
            <v-row justify="end">
              <v-btn
        ...
        <v-card-text class="pb-0">
          ...
          <v-row>                      
            <v-col class="d-flex" cols="6" sm="8">
              <v-autocomplete
                label="Equivalencia"
                v-model="form.equivalencia"
                :items="equivalencias"
                :item-text="format_equivalencia"
                item-value="codigo_agc"
                hide-details
                outlined
                return-object
                @change="errores.equivalencia = false"
                :error="errores.equivalencia"
              ></v-autocomplete>
            </v-col>
          </v-row>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Equivalencia", align: "center", value: "eqbase_rep" },
        { text: "Nombre Equivalencia", align: "left", value: "descrbase_rep" },        
  ...
  methods: {
    editar_item(data) {
      ...
      var busqueda = this.equivalencias.find(
        (el) => el.llavebase_rep == parseFloat(item.eqbase_rep)
      );
      this.form.equivalencia = busqueda

      var busq_agenc = this.agencias.find(
        (el) => el.codigo_agc == parseFloat(item.agencia_rep)
      );
      this.form.agencia = busq_agenc;
    ...
    format_envio(data) {
      ...
      var agencia = data.agencia.codigo_agc;
      var equivalencia = data.equivalencia.llavebase_rep;
      var eliminar = data.eliminar ? "1" : "0";
      let datos = {
        url: "Financiero/dlls/CfEqbasegth.dll",
        data:
    ...
    cargarcontenido() {
      ...
      post
        .postData({
          url: "Financiero/dlls/CfEqbasegthJ.dll",
        ...
        .catch((err) => {
          ...
          this.$emit("snack", {
            color: "error",
            text: "Error al cargar Equivalencias",
            estado: true,
          });
        });
    },
    ...
    cargarbases() {
      ...
      post
        ...
        .then((data) => {
          this.loader = false;
          this.equivalencias = JSON.parse(JSON.stringify(data));
        })
        .catch((err) => {
          this.loader = false;
          this.$emit("snack", {
    ...
    format_equivalencia: function (val) {
