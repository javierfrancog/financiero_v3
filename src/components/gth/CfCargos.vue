    ...
    <v-flex xs12 md12>
      <v-card class="mx-auto col-12" max-width="1300" elevation="2">
        <v-card-title>
          <v-list-item>
            <v-list-item-icon>
              <v-icon size="30" color="blue darken-4 ">mdi-bag-checked</v-icon>
            </v-list-item-icon>
            <v-list-item-content>
        ...
        <v-card-text class="pb-0">
          ...
          <v-row justify="center">
            <v-col class="d-flex" cols="3">
              <v-switch v-model="form.lider" label="Cargo Lider"></v-switch>
            </v-col>
            <v-col class="d-flex" cols="12" sm="8" v-if="form.lider">
              <v-autocomplete
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Codigo", align: "left", value: "codigocargo_rep" },
        { text: "Descripción", align: "left", value: "descrcargo_rep" },
        { text: "Estado", align: "center", value: "estadocargo_rep" },
      ...
      card_estado: false,
      clasificaciones: [
        { text: "Administración", value: 1 },
        { text: "Operativo", value: 2 },
        { text: "Comercial", value: 3 },
        { text: "Producción", value: 4 },
      ],
  ...
  methods: {
    ...
    editar_item(data) {
      ...
      var busqueda = this.clasificaciones.find(
        (el) => el.value == parseFloat(item.tipocargo_rep)
      );
      this.form.clasifica = busqueda || null;
      this.form.lider = false;
      if (item.idlider_rep > 0) {
        this.form.lider = true;
      }
      busqueda = this.ruts3.find(
        (el) => el.identificacion_rut == item.idlider_rep
      );
      this.form.colabora = busqueda;

      this.dialogo.estado = true;
      this.dialogo.titulo = "Modificar Registro";
      this.dialogo.tipo = 1;
      this.form.codigo = item.codigocargo_rep;
      this.form.descripcion = item.descrcargo_rep;
    ...
    guardar: function () {
      ...
      } else if (data.lider && !data.colabora) {
        this.errores.colabora = true;
        this.send_error("Seleccione un lider!");
      } else {
    ...
    format_envio(data) {
      ...
      var lider = data.colabora ? data.colabora.identificacion_rut : 0;
      var estado = data.estado ? "1" : "0";
      var eliminar = data.eliminar ? "1" : "0";

      let datos = {
        url: "Financiero/dlls/Cfcargos.dll",
        data:
