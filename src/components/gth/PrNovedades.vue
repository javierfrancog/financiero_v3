          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Novedades de Nómina</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
...
export default {
  ...
  methods: {
    ...
    agregar_item() {
      this.init_form();
      if (!this.form.colabora) {
        this.errores.colabora = true;
        this.send_error("Seleccione un Colaborador!");
      } else {
        this.dialogo_novedad.estado = true;
        this.dialogo_novedad.titulo = this.dialogo_novedad.tipo = 0;
        this.dialogo_novedad.titulo =
          this.form.colabora.Apellido1colabora_rep.trim() +
          " " +
          this.form.colabora.nombrescolabora_rep.trim();
        this.loader = true;
        post
          .postData({
    ...
    editar_item(item) {
      ...
      this.form.fechafin = item.fechafin_rep.trim();
    ...
    guardar_prestamo() {
      ...
      if (!data.novedad) {
      ...
      } else if (!data.fechainidesc) {
        this.errores.fechainidesc = true;
        this.send_error("Fecha Obligatoria!");
      } else if (data.valorprest < 1) {
        this.errores.valorprest = true;
        this.send_error("Valor Obligatorio!");
      } else if (!data.tipoliq) {
        this.errores.tipoliq = true;
        this.send_error("Selecciona Forma Pago!");
      } else if (data.tcuotas < 1) {
        this.errores.tcuotas = true;
        this.send_error("Total cuotas Obligatorio!");
      } else {
        this.card_estado = true;
        this.dialogo_novedad.estado = false;
        let periodo = data.fecha.replace(/\-/g, "");
        ...
        let novedad = data.novedad.llavebase_rep;
        let consecutivo = data.consecutivo;
        let fechainidesc = data.fechainidesc.replace(/\-/g, "");
        let tcuotas = data.tcuotas.trim();
        let valor = data.valorprest.replace(/\,/g, "");
        let soporte = "";
        let observacion = "";
        let fechafin = "";
        let estado = 0;
    ...
    get_basico: function () {
      post
        .postData({
          ...
          data: sessionStorage.Sesion + "|" + "0" + "|" + "0" + "|" + 1 + "|" ,
          method: "",
        })
        .then((data) => {
