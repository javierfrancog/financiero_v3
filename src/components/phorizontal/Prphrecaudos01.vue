          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline ml-2"
                >Registro de Pagos</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
      ...
      <v-card class="pa-2">
        ...
        <v-card-text class="pb-0">
                ...
                <v-col>
                  <v-data-table
                    ...
                    :headers="[
                      ...
                      { text: 'Concepto', value: 'descrcargo_cart' },
          ...
          <v-row class="mt-1">
            ...
            <v-col class="d-flex pb-0" cols="8">
              <v-autocomplete
  ...
  methods: {
    guardar_tabla() {
      ...
      if (recibido < 0) {
        ...
        let rteica = 0;
        let rteestamp = 0;
        let anticipo = parseFloat(this.form.anticipo) || 0;
        let dctos = parseFloat(this.form.dctos) || 0;

        let otros_debitos = [
          {
            codigoCuenta: "4175050101",
            valor: dctos,
          },
        ];
        let otros_creditos = [
          {
            codigoCuenta: "2805050101",
            valor: anticipo,
          },
          {
        ...
        detalle.forEach((item) => {
          ...
          let item_cartera = item.item_cart;
          let cuenta_cartera = this.cuentasContables.find(
            (el) => el.cod_puc == item.cta_cart
          );

          let prefijo = item.pref_cart.padStart("4", " ");
          let obj = {
            centroCosto: "1001",
            codigoCuenta: cuenta_cartera.cod_puc,
            comprobante: {
              rut: cliente,
            },
            creditos,
        ...
        if (recibido) {
          let cuenta_cartera = this.cuentasContables.find(
            (el) => el.cod_puc == this.comprobanteEditar.cuentaPago.cod_puc
          );
    ...
    async calcular_diferencia() {
      ...
      let rteica = 0;
      let rteestamp = 0;
      let anticipo = this.form.anticipo || 0;
      let dctos = parseFloat(this.form.dctos) || 0;
    ...
    seleccionar_item(item) {
      ...
      let existe = this.dialogo_recaudo.tabla_seleccionados.find(
        (el) =>
          ...
          el.item_cart.trim() == item.item_cart.trim()
      );
      if (!existe) {
        ...
        this.get_facturas_csv().then((facturas) => {
          ...
          reader.onload = function () {
            ...
            array.forEach((el, index) => {
              ...
              var documento = +parse_documento[0];
              var busqueda_factura = facturas.find(
                (fac) => fac.numero_cart.trim() == documento
              );
    ...
    grabar_registro() {
      ...
      if (!data.tipoDocumento) {
        ...
        this.get_plano(detalle_format)
          .then((data) => {
            datosEnvio += data + "|" + tiponumer + "|" + nrorotativo + "|";
            post
              .postData({
                url: "Phorizontal/dlls/PrRegcont.dll",
                data: datosEnvio,
                method: "",
              })
              .then((data) => {
    ...
    getDetalleConcepto() {
      ...
      if (codigo === "0002") {
        ...
        var busqueda_cta = this.cuentasContables.find(
          (el) => el.cod_puc.trim() == "1110050101"
        );
        this.comprobanteEditar.cuentaPago = busqueda_cta;
      }
    ...
    llenarCreditos(comprobante) {
      ...
      if (concepto.codigo_conc == "0001") {
        ...
        if (comprobante.conc_conta.ctaiva_conc.descripcta_conc) {
          this.detalle.push({
            codigoCuenta: comprobante.conc_conta.ctaiva_conc.codigo_conc,
            descripcionCuenta:
              comprobante.conc_conta.ctaiva_conc.descripcta_conc,
        ...
        // Agregar crédito
        var valorCredito = valorDebito + iva - valorRetencion - valorReteica;
        // var valorCredito = valor;
        if (valorCredito > 0) {
          this.detalle.push({
            codigoCuenta: comprobante.conc_conta.ctacre_conc.codigo_conc,
            descripcionCuenta:
        ...
        if (facturas && facturas.length > 0) {
          facturas.forEach((item) => {
            ...
            var verificar = this.detalle.find(
              (el) => el.documento == key_factura
            );
            var cuentaFacturas = this.cuentasContables.find((e) => {
              return e.cod_puc.trim() == item.cuenta_cart.trim();
            });
            var cuentainteres = this.cuentasContables.find((e) => {
              return e.cod_puc.trim() == "41501001";
            });
            var cuentaseguro = this.cuentasContables.find((e) => {
              return e.cod_puc.trim() == "24101002";
            });

            var cuentamora = this.cuentasContables.find((e) => {
              return e.cod_puc.trim() == "41501002";
            });
            if (!verificar) {
              let capital = item.vlrcapital_cart.replace(/\,/g, "") || 0;
              let intercte = item.vlrinteres_cart.replace(/\,/g, "") || 0;
              let intermora = item.vlrintermora_cart.replace(/\,/g, "") || 0;
              let seguro = item.vlrseguro_cart.replace(/\,/g, "") || 0;
              ...
              this.detalle.push({
                codigoCuenta: cuentaseguro.cod_puc,
                descripcionCuenta: cuentaseguro.descrip_puc,
                documento: key_factura,
                rut,
                centroCosto,
                debitos: 0,
              ...
              this.detalle.push({
                codigoCuenta: cuentainteres.cod_puc,
                descripcionCuenta: cuentainteres.descrip_puc,
                documento: key_factura,
                rut,
                centroCosto,
                debitos: 0,
              ...
              this.detalle.push({
                codigoCuenta: cuentamora.cod_puc,
                descripcionCuenta: cuentamora.descrip_puc,
                documento: key_factura,
                rut,
                centroCosto,
                debitos: 0,
    ...
    format_factPendientes: function (val) {
      return `${
        val.pref_cart
      }${val.numero_cart.trim()} - ${val.valor_cart.trim()} - Fecha: ${val.fecha_cart.trim()}- Cuota: ${val.nrocuota_cart.trim()}`;
    },
    get_facturas_csv() {
    ...
    get_facturas() {
      ...
      let url_envio = "Phorizontal/dlls/PrCarteraJ.dll";
    ...
    cargarTablaconceptos: function () {
      this.conceptosc.unshift({
        codigo_conc: "0002",
        agencia_conc: "0000",
        descrip_conc: "ABONO/PAGO CLIENTE",
      });
    },
    clear_tabla_rec() {
