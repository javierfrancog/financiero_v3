          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Buscar Pacientes por Apellido cl</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
                    ...
                    <v-col class="d-flex" cols="6">
                      <v-select
                        :items="[
                          ...
                          {
                            text: 'Especial o Excepcion beneficiario',
                            value: 7,
                          },
                          {
          ...
          >
            <template v-slot:default>
              <PrPacientes
                ...
                :paciente-inicial="pacienteSeleccionado"
                @paciente-guardado="guardarPaciente"
...
export default {
  ...
  methods: {
    abrirFormularioPaciente() {
      // Limpia el paciente para agregar uno nuevo
    ...
    guardarPaciente(paciente) {
      // Aquí haces lo que necesites al guardar el paciente
      console.log("Paciente guardado:", paciente);
      // Por ejemplo, podrías guardarlo en un array o enviarlo al servidor
      this.mostrarFormularioPaciente = false;
    },
    ...
    get_excel() {
      ...
      post
        .postData({
          url: "Clinico/dlls/Clcfpaciente01J.dll",
    ...
    opcionDialog(opc, key) {
      ...
      this.form.paciente = this.id_paciente;
      if (opc == "C") {
        this.dialog = true;
      } else {
        ...
        if (item.length) {
          ...
          this.form.tipoid = { value: item.tipoid };
          ...
          let busqueda_select = this.eps.find(
            (el) => el.value.trim() == item.eps.trim()
          );
          this.form.eps = busqueda_select;

          this.form.estadoCivil = { value: parseInt(item.estadoCivil) };
          this.form.regimen = { value: parseInt(item.regimen) };
          this.form.tipousuario = { value: parseInt(item.tipousuario) };
