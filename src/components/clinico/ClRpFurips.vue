...
export default {
  ...
  methods: {
    ...
    imprimir_formato() {
      ...
      let nro = this.item_select.nro_rep.trim();
      let fecha = this.item_select.fecha2_rep;
      ...
      post
        ...
        .then((data) => {
          ...
          for (let i = 1; i <= 10; i++) {
            detalleEvento_base64 +=
              data_impresion[`desc${i.toString().padStart(2, "0")}_evt_rep`];
          }
    ...
    guardar() {
      ...
      let consec = parseFloat(this.item_select.nro_rep);
      let fecha = this.item_select.fecha2_rep;
      post
        .postData({
          url: "Clinico/dlls/PrFurips02.dll",
          data: sessionStorage.Sesion + "|" + consec + "|" + fecha + "|" + factura + "|",
          method: "",
        })
        .then((data) => {
          this.loader = false;
          ...
          this.imprimir_formato();
        })
        .catch((err) => {
          this.loader = false;
          this.$emit("snack", {
            color: "error",
            text: "Error al guardar Furips",
            estado: true,
          });
        });
    },
