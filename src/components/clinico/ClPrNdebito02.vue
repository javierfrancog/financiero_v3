          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Reimpresión Nota Débito Cl</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
...
export default {
  ...
  methods: {
    ...
    async send_dian_titan(item) {
      ...
      post
        .postData({
          url: "PtoVta/dlls/PrNdebTitanJ.dll",
          data: datosEnvio,
          method: "",
        })
        .then((data) => {
    ...
    envio_json_titan(factura) {
      let nombre64 = " ";
      let urlEnvio = "https://server1ts.net/financiero/inc/api_dian/ndelect.titan.php";

      let detalles = factura.debit_note_lines.map((item) => {
