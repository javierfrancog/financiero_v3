    ...
    <v-flex xs12 md12>
      ...
      >
        <v-card-title>
          <v-list-item>
            <v-list-item-icon>
              <v-icon size="30" color="blue darken-4 ">mdi-briefcase-check-outline</v-icon>
            </v-list-item-icon>
            <v-list-item-content>
              <v-list-item-title class="headline">Reporte Proveedores</v-list-item-title>
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
        <div class="pa-0 px-8">
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Descripción", align: "center", value: "descriprut_rep" },
        { text: "Factura", align: "left", value: "docexter_rep" },        
    ...
    generar_excel() {
      ...
      data.forEach((el, index)=>{
        if (el.rut_rep != undefined && el.saldo_rep != undefined){
          console.log("proveedor", el.descriprut_rep);
          if (rut === el.rut_rep){
            poneSubTot = false;
          }else{
            if (index > 0){
          ...
          data_parse.push({
            descriprut_rep: (el.descriprut_rep === undefined)?"":el.descriprut_rep.trim(),
            docexter_rep: (el.docexter_rep === undefined)?"":el.docexter_rep.replace(/\ /g, ""),
            fecha_rep: (el.fecha_rep === undefined)?"":el.fecha_rep.trim(),
            diasvenc_rep: (el.diasvenc_rep === undefined)?0:parseInt(el.diasvenc_rep.replace(/\,/g, "").replace(/\ /g, "")),
            diasxvenc_rep: (el.diasxvenc_rep === undefined)?0:parseInt(el.diasxvenc_rep.replace(/\,/g, "").replace(/\ /g, "")),
            saldo_rep: (el.saldo_rep === undefined)?0:parseFloat(el.saldo_rep.replace(/\,/g, "").replace(/\ /g, "")),
          });
        }
      });
