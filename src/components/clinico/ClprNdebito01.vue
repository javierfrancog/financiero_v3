          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Nota Débito cl</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
        ...
        <v-card-actions>
          <v-row class="d-flex justify-center col-12">
            <v-col cols="4">
              <v-select
                :items="[
                  { text: 'Menor Valor Cobrado', value: 1 },
                  { text: 'Ajuste de Precio', value: 2 },
                  { text: 'Otro', value: 3 },
                ]"
                label="Causal"
                outlined
...
export default {
  ...
  methods: {
    ...
    get_enviardian() {
      ...
      console.log("Comprobante", this.print.comprobante);
    },
    ...
    grabar_cufe(res) {
      ...
      post
        .postData({
          url: "Ptovta/dlls/PrDebito01Id.dll",
          data: datosEnvio,
          method: "",
        })
        .then((data) => {
      ...
      if (!data.clientes) {
        ...
        this.get_plano(detalle)
          .then((plano) => {
            ...
            post
              .postData({
                url: "Clinico/dlls/PrNDebito01.dll",
                data: datosEnvio,
                method: "",
              })
              .then((data) => {
