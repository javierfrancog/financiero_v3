          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Reporte Libro de Viajes Propietarios</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Avances", align: "center", value: "avances_lvia" },
  ...
  methods: {
    ...
    print_reporte() {
      ...
      data.forEach((el) => {
        if (el.id_lvia) {
          let item = [
            el.fecha_lvia.trim(),
            el.id_lvia.trim(),
            el.descagencia_lvia.trim(),
            el.fpago_lvia.trim(),
            el.subtotal_lvia.trim(),
            el.seguro_lvia.trim(),
            el.total_lvia.trim(),
            el.avances_lvia.trim(),
            el.origen_lvia.trim(),
            el.destino_lvia.trim(),
            el.descuser_lvia.trim(),                                                                                    
          ...
          total_subtotal += parseFloat(el.subtotal_lvia.trim().replace(/\,/g, ""))
          total_avances += parseFloat(el.avances_lvia.trim().replace(/\,/g, ""))
          total_seguro += parseFloat(el.seguro_lvia.trim().replace(/\,/g, ""))
          total_total += parseFloat(el.total_lvia.trim().replace(/\,/g, ""))
        } else {
          data_parse.push(["\n\n", "", "", ]);
        }
      });
      data_parse.unshift([
        ...
        { text: "Libro", style: "header_table" },
        { text: "Agencia", style: "header_table" },
        { text: "F.Pago", style: "header_table" },
        { text: "Subtotal", style: "header_table" },
        { text: "Seguro", style: "header_table" },
        { text: "Total", style: "header_table" },
        { text: "Avances", style: "header_table" },        
        { text: "Origen", style: "header_table" },
        { text: "Destino", style: "header_table" },
        { text: "Despachador", style: "header_table" },        
      ]);
      data_parse.push([
        { text: "", style: "header_table" },
        { text: "", style: "header_table" },
        { text: "", style: "header_table" },
        { text: "TOTALES: ", style: "header_table" },
        { text: this.formatNumero(total_subtotal), style: "header_table" },
        { text: this.formatNumero(total_seguro), style: "header_table" },
        { text: this.formatNumero(total_total), style: "header_table" },
        { text: this.formatNumero(total_avances), style: "header_table" },
      ...
      this.pdfs.getBase64(logo_src).then((logo) => {
        this.pdfs._informe({
          ...
          logo,
          titulo: "Reporte Libro de Viajes -Propietarios-",
          fontSize: 8,
        });
      });
    },    
    ...
    consultar_lviajes() {
      ...
      if (!data.placas) {
        ...
        var datos = {
          url: "Transporte/dlls/RPLViajesVJ.dll",
          data:
