          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Resumen Ventas por dia/Usuario</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
        ...
        <v-row>
          <v-col class="d-flex" cols="4" md="4">
            <v-autocomplete
              :items="taquilleros"
              label="Taquillero"
              v-model="form.taquillero"
              :item-text="format_taquillero"
...
export default {
  ...
  data() {
    return {
      ...
      libro: {},
      headers: [
        { text: "Taquillero", align: "left", value: "nombreusuario_rep" },
  ...
  methods: {
    ...
    consultar_reporte() {
      console.log(this.form.taquillero)
      if (this.form.taquillero) {
        this.loader = true;
        let fechaInicial = this.form.fechaInicial.replace(/\-/g, "");
        let fechaFinal = this.form.fechaFinal.replace(/\-/g, "");
        let taquillero = this.form.taquillero.idusuario_rep;

        var datos = {
          url: "Transporte/dlls/RpResVtasuserJ.dll",
          data:
        ...
        post
          ...
          .catch((error) => {
            this.loader = false;
            this.send_error("Error al consultar taquilleros");
          });
      } else {
        this.errores.puntos = true;
        this.send_error("Debe seleccionar un taquillero");
      }
    },
    cargartaquilleros: function () {
      this.loader = true;
      post
        .postData({
          url: "Transporte/dlls/RpusuariosJ.dll",
          data: sessionStorage.Sesion + "|",
          method: "",
        })
        .then((data) => {
          this.loader = false;
          this.taquilleros = data;
          this.taquilleros.unshift({
    ...
    format_taquillero(val) {
      return `${val.Apellido1_rep} - ${val.nombres_rep}`;
    },
