          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Reporte de Documentos Electronicos</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
        <div class="pa-0 px-8">
          <v-card-text class="px-0 pa-0">
            ...
            <v-data-table
              ...
              :search="search"
            >
            <template v-slot:item.idpropietario_rep="{ item }">
                {{ parseInt(item.idpropietario_rep) }}
              </template>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        { text: "Propietario", align: "center", value: "idpropietario_rep" },
        { text: "Placa", align: "center", value: "placa_rep" },
        { text: "Total Tiquetes", align: "center", value: "cantidad_rep" },
        { text: "Vlr_Cobro", align: "center", value: "total_rep" },
      ],
      errores: {
        placas: false,
      },
  ...
  methods: {
    print_reporte_excel() {
      ...
      var data_parse = data.map((el) => {
        ...
        let subtotal = parseFloat(el.total_rep.replace(/\,/g, "")) || 0;

        el.total_rep = subtotal;
        return el;
      });

      var columnas = [
      {
          title: "Id Propietario",
          value: "idpropietario_rep",
        },
        {
          title: "Vehiculo",
          value: "placa_rep",
      ...
      var header_format = [
        { text: "REPORTE PROVISION DOCUMENTOS ELECTRONICOS", bold: true, size: 14 },
      ...
      this.pdfs.getBase64(logo_src).then((logo) => {
        this.excel
          ._informe({
            sheetName: "Reporte Resumen docs electronicos",
            ...
            archivo: `REPORTE DOCS ELECTRONICOS-${new Date().getTime()}`,
          })
          .then((data) => {
    ...
    consultar_reporte() {
      ...
      this.postData({
        url: "Transporte/dlls/RpDocselectJ.dll",
        data: data.join("|") + "|",
        method: "",
      })
        .then((data) => {
          ...
          this.contenido.forEach((el) => {
            let valor = parseFloat(el.total_rep.replace(/\,/g, "")) || 0;
            this.total_efectivo += valor;
          });
    ...
    get_plano(detalle) {
      return new Promise((resolve, reject) => {
        ...
        detalle.forEach((el) => {
          var base =
            parseFloat(el.subtotal_lvia.toString().replace(/\,/g, "")) || 0;
          var vlr_ica =
            parseFloat(el.vlrica_lvia.toString().replace(/\,/g, "")) || 0;

          nuevo.push({
            placa: el.placa_lvia || "",
            base: base,
            ica: vlr_ica,
            propietario: parseFloat(el.idpropietario_lvia) || "0",
          });
        });

        var formData = new FormData();
        formData.append("tipo", "Ica");
