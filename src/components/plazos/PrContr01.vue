          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Generación Contratos</v-list-item-title
              >
            </v-list-item-content>
            <v-card-actions>
              <v-btn
          ...
          <v-card-text class="px-0 pa-0">
            ...
            <v-row>
              <v-card-subtitle
                ...
                >Cierre de Contrato</v-card-subtitle
              >
            </v-row>
            <v-row>
                ...
                >
                  <template v-slot:activator="{ on }">
                    <v-text-field
                      ...
                      outlined
                      v-on="on"
                      :error="errores.fecha_ini"
                      @input="errores.fecha_ini = false"
                    ></v-text-field>
                  </template>
                  <v-date-picker
                    v-model="form.fecha_ini"
  ...
  methods: {
    ...
    async grabar_factura() {
      ...
      if (!data.punto_fact) {
        ...
        this.send_error("Debe indicar el numero de Cuotas");
      } else if (!data.fecha_ini) {
        this.errores.fecha_ini = true;
        this.send_error("Debe indicar la fecha de inicio de Cuota");
      } else {
        ...
        this.get_plano(detalle)
          .then((plano) => {
            ...
            var nrorem = "0";
            var correos = "";
            let fechaini = data.fecha_ini.replace(/\-/g, "");
            ...
            post
              .postData({
                url: "Plazos/dlls/PrContrato.dll",
                data: datosEnvio,
                method: "",
              })
              .then((data) => {
              ...
              .catch((err) => {
                ...
                this.disabled.btn_enviar = false;
                this.disabled.grabar_factura = true;
                this.send_error("Error al grabar Contrato");
              });
          })
          .catch((err) => {
            this.disabled.btn_enviar = false;
    ...
    agregar_servicio() {
      ...
      if (
      ...
      } else if (!this.form.umedida && this.empresa.ccostofact_empr == "1") {
        ...
        this.articulo_select.unidadmed = this.form.umedida
          ? this.form.umedida
          : "0";
    ...
    format_clientes(val) {
      ...
      } - ${val.descripcion_rut.trim()}- ${val.nombrecontac_rut}`;
    },
    ...
    get_consecutivo() {
      ...
      post
        .postData({
          url: "Plazos/dlls/PrContratoN.dll",
          data:
            sessionStorage.Sesion +
            "|" +
