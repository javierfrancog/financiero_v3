          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline ml-2"
                >Registro Inventario Cíclico</v-list-item-title
              >
            </v-list-item-content>
...
export default {
  ...
  data() {
    return {
      ...
      headers_tabla_saldos: [
        ...
        { text: "Cant Conteo", align: "center", value: "cantsolicita_rep" },
  ...
  methods: {
    get_saldo() {
      ...
      let agencia = this.form.ubica_destino;
    ...
    add_item(item) {
      ...
      let cantidad = item.cantsolicita_rep
        ...
        : 0;
      let saldo = item.cant_rep ? parseFloat(item.cant_rep) : 0;
    ...
    async get_medicamento() {
      ...
      let ubicacion = this.form.ubica_destino;
      let molecula = this.form.molecula.codigo_rep;
      this.medicamentos = [];
      await post
        .postData({
          url: "Dispensa/dlls/PrCiclico01J.DLL",
          data: sessionStorage.Sesion + "|" + molecula + "|" + ubicacion + "|",
          method: "POST",
        })
        .then((data) => {
          data.pop();
    ...
    format_envio(data) {
      ...
      this.tablaProduccion.datos.forEach((item, index) => {
        ...
        let saldo = parseFloat(item.tab_saldo);
        let lote = item.tab_lote_.trim();
        let vence = item.tab_vence.replace(/\-/g, "");
        tablaOrigen[
          `DATOJ-${format_index}`
        ] = `${codigo}|${cantidad}|${lote}|${vence}|${saldo}|`;
      });
      ...
      let importarhtml =
        ...
        "|";
      let datos = {
        url: "Dispensa/dlls/PrCiclico.dll",
    ...
    async get_impresion() {
      ...
      post
        .postData({
          url: "Dispensa/dlls/PrCiclicos01J.dll",
          data: datosEnvio,
          method: "",
        })
        .then(async (data) => {
    ...
    get_consecutivo() {
      ...
      if (agencia) {
        ...
        post
          .postData({
            url: "Dispensa/dlls/PrCiclicoN.dll",
    ...
    format_medicam(val) {
      return `${val.descripcion.trim()} - ${val.descriplaboratorio_rep.trim()}`;
    },
