          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Homologación de Productos</v-list-item-title
              >
            </v-list-item-content>
            <v-row justify="end">
              <v-btn
    ...
    <v-dialog v-model="dialogo.estado" persistent max-width="900px">
      <v-card class="px-6" :loading="card_estado" :disabled="card_estado">
        ...
        <v-card-text class="pb-0">
          ...
          <v-row>
            <v-col class="d-flex" cols="12" v-if="this.form.producto">
              <v-autocomplete
                outlined
                label="Producto que Homologa"
                v-model="form.homologado"
                :item-text="format_medicamento"
                :items="medicamentos"
                :error="errores.homologado"
                @change="errores.homologado = false"
                return-object
                clearable
              ></v-autocomplete>
            </v-col>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Descripción", align: "left", value: "descorigen_rep" },
        { text: "Homologado", align: "left", value: "deschomologa_rep" },        
  ...
  methods: {
    editar_item(data) {
      ...
      let busqueda = this.medicamentos.find(
        (el) => el.cod_rep == item.codorig_rep
      );
      this.form.producto = busqueda;

       busqueda = this.medicamentos.find(
        (el) => el.cod_rep == item.codhomo_rep
      );
      this.form.homologado = busqueda;
    ...
    guardar: function () {
      ...
      } else if (!data.homologado) {
        this.errores.homologado = true;
        this.send_error("Homologación Obligatoria!");
      } else {
    ...
    format_envio(data) {
      ...
      var codigo = data.producto.cod_rep;
      var homologa = data.homologado.cod_rep;
      var eliminar = data.eliminar ? "1" : "0";
      let datos = {
        url: "Dispensa/dlls/CfHomologa.dll",
        data:
    ...
    cargarcontenido() {
      ...
      post
        .postData({
          url: "Dispensa/dlls/CfHomologaJ.dll",
