    ...
    <v-flex xs12 md12>
      <v-card class="mx-auto col-12" max-width="1300" elevation="2">
        <v-card-title>
          <v-list-item>
            <v-list-item-icon>
              <v-icon size="30" color="blue darken-4 ">mdi-flask-empty-plus-outline</v-icon>
            </v-list-item-icon>
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Configuración Laboratorios</v-list-item-title
              >
            </v-list-item-content>
            <v-row justify="end">
              <v-col>
...
export default {
  ...
  methods: {
    print_reporte_excel() {
      ...
      var header_format = [
        { text: "REPORTE LABORATORIOS", bold: true, size: 16 },
      ];
      ...
      this.pdfs.getBase64(logo_src).then((logo) => {
        this.excel
          ._informe({
            sheetName: "Reporte Laboratorios",
            ...
            archivo: `REPORTE LABORATORIOS-${new Date().getTime()}`,
          })
          .then((data) => {
    ...
    format_envio(data) {
      ...
      let datos = {
        url: "Dispensa/dlls/CfLaboratorio.dll",
        data:
    ...
    agregar_item() {
      ...
      post
        .postData({
          url: "Dispensa/dlls/CfLaboratorioN.dll",
