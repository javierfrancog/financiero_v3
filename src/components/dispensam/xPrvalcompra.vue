  ...
  methods: {
    ...
    registrar() {
      const repetido = this.contenidofact.some(
        (el) => parseFloat(el.cod_med) == parseFloat(this.medicamento.cod_rep)
      );
      console.log(this.medicamento, this.contenidofact)
    ...
    async consultar_xml() {
      ...
      let response = await fetch(
        "https://server3ts.com//financiero/inc/acuse.php",
        {
          method: "POST",
          body: data,
        }
    ...
    async rechazo_item() {
      ...
      let response = await fetch(
        "https://server3ts.com//financiero/inc/getFile.drive.php",
        {
          method: "POST",
          body: data,
        }
    ...
    async acuse_rechazo(item, evento, motivo, base64) {
      ...
      let response = await fetch(
        "https://server3ts.com//financiero/inc/eventos.dian.php",
        {
          method: "POST",
          body: data,
        }
    ...
    async consultar_test() {
      ...
      let response = await fetch(
        "https://server3ts.com//financiero/inc/acuse_test.php",
        {
          method: "POST",
          body: data,
        }
    ...
    async pdffact_item(item, evento) {
      ...
      if (response.type === "pdf") {
        const url = `https://server3ts.com//tmp/${response.fileName}`;
        window.open(url, "_blank").focus();
    ...
    tipo_xml_1(xml) {
      ...
      // console.log("Salto_5:", xml_factura);
      ...
      for (let i = 0; i < invoiceLines.length; i++) {
        ...
        if (note) {
          ...
          if (noteArray[0] == "") {
            console.log("Salto_note:", noteArray);
    ...
    async grabar_contabilidad() {
      ...
      let vlriva19 = this.form.vlriva19 ? this.form.vlriva19 : 0;
      let vlriva5 = this.form.vlriva5
        ? this.form.vlriva5.replace(/\,/g, "")
        : 0;
      let vlrretefte = this.form.vlrretefte
        ? this.form.vlrretefte.replace(/\,/g, "")
        : 0;
      let vlrrteica = 0;

      console.log("grabar", this.form);

      let vlrcompra = this.form.subtotal ? this.form.subtotal : 0;
      let vlrpasivo = 0;
