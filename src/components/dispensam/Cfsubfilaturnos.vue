    ...
    <v-flex xs12 md12>
      <v-card class="mx-auto col-12" max-width="1300" elevation="2">
        <v-card-title>
          <v-list-item>
            <v-list-item-icon>
              <v-icon size="30" color="blue darken-4 ">mdi-reddit</v-icon>
            </v-list-item-icon>
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Configuración SubFilas Turnero</v-list-item-title
              >
            </v-list-item-content>
            <v-row justify="end">
              <v-btn
...
export default {
  ...
  methods: {
    ...
    format_envio(data) {
      ...
      let idturno = "";
      let filappal = data.filappal.cod_rep;
    ...
    cargarfilas() {
      ...
      post
        ...
        .then((data) => {
          this.loader = false;
          data.pop();
          this.filasppal = JSON.parse(JSON.stringify(data));
        })
        .catch((err) => {
          this.loader = false;
          this.$emit("snack", {
