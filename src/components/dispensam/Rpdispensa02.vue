          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Resumen Dispensación</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
  ...
  methods: {
    ...
    print_reporte_excel() {
      ...
      data.forEach((el) => {
        ...
        copia.cant_rep = parseFloat(el.cant_rep.replace(/\,/g, "")) || 0;
      ...
      var header_format = [
        { text: "   RESUMEN DISPENSACIONES  .", bold: true, size: 14 },
      ...
      this.pdfs.getBase64(logo_src).then((logo) => {
        this.excel
          ._informe({
            sheetName: "Resumen Dispensaciones",
            ...
            archivo: `RESUMEN DISPENSACIONES-${new Date().getTime()}`,
          })
          .then((data) => {
    ...
    consultar_reporte() {
      if (!this.form.puntos) {
        ...
        this.postData({
          url: "dispensa/dlls/RpDispensa04J.dll",
          data: data.join("|") + "|",
          method: "",
        })
          .then((data) => {
            ...
            this.total = 0;
            data.forEach((item) => {
              let cantidad = parseFloat(item.cant_rep.replace(/\,/g, "")) || 0;
              this.total += cantidad;
            });

            this.graficar(this.contenido);
          })
          .catch((err) => {
            this.loader = false;
    ...
    async graficar(data) {
      ...
      return new Promise((resolve) => {
        this.chartObj = new Chart(document.getElementById("grafico1"), {
          type: "bar",
          data: {
            ...
            datasets: [
              {
                ...
                label: "Cantidad",
                data: data.map((sede) => parseFloat(sede.cant_rep)),
          ...
          options: {
            ...
            title: {
              display: true,
              text: "Resumen dispensaciones por sede",
              fontSize: 18,
              fontColor: "black",
              fontStyle: "bold",
            },
        ...
        // let url = this.chartObj.toBase64Image();
        resolve(url);
      });
    },
    send_error(msj) {
