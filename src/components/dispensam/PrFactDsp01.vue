            ...
            >
              ...
              <template v-slot:item.edit="{ item }">
                ...
                <v-tooltip top>
                  <template v-slot:activator="{ on }">
                    <v-btn
                      @click="get_factura(item)"
                      color="success"
                      fab
                      small
                      icon
                      :disabled="parseFloat(item.factura_rep) > 0 || item.descnovedad_rep.trim() == 'Sin Validar' || item.descnovedad_rep.trim() == 'Rechazado'"
                      v-on="on"
                    >
                ...
                <v-tooltip top>
                  <template v-slot:activator="{ on }">
                    <v-btn
                      @click="imp_factura(item)"
                      color="red accent-3"
                      fab
                      small
                      icon
                      v-on="on"
                      :disabled="parseFloat(item.factura_rep) == 0"
                    >
                      <v-icon>mdi-file-pdf-box</v-icon>
                    </v-btn>
                  </template>
                  <span>Imprmir Factura</span>
                </v-tooltip>
              </template>
            </v-data-table>
          </v-card-text>
    ...
    <v-dialog v-model="dialogovalida.estado" max-width="900px">
      <template>
        <v-card class="px-6" :loading="card_estado" :disabled="card_estado">
          <v-card-title>
            <span class="title col-12">Validación Dispensación</span>
          </v-card-title>
          <v-card-text>
            <v-row class="d-flex">
              <v-col class="d-flex" cols="4">
...
export default {
  ...
  data() {
    return {
      ...
      empresa: [],
      novedad: [
        { text: "Paciente Inactivo", value: 1 },
        { text: "Fórmula Médica no Cumple", value: 2 },
        { text: "Autorización Vencida", value: 3 },
        { text: "Medicamento Requiere Autorización", value: 4 },
        { text: "Error en Autorización", value: 5 },
        { text: "Paciente Inactivo", value: 1 },
      ],
      habilita_validar: false,
      habilita_novedad: false,
      ...
      headers: [
        ...
        { text: "Operador", align: "center", value: "userevento_rep" },
  ...
  methods: {
    get_factura(item) {
      console.log(item);
      var fecha = item.fecha_rep.split("-").join("");
      var sede = item.idagenc_rep;
      var numero = parseFloat(item.consec_rep);
      let contrato = parseFloat(item.contrato_rep);
      ...
      post
        .postData({
          url: "Dispensa/dlls/PrFactptes.dll",
          data: datos,
          method: "POST",
        })
        .then((data) => {
    ...
    get_prefactura(item) {
      ...
      post
        .postData({
          url: "Dispensa/dlls/PrPrefact01J.dll",
          data: datosEnvio,
          method: "",
        })
        .then(async (data) => {
        ...
        .catch((err) => {
          console.log(err);
          this.send_error("Error al imprimir Prefactura");
        });
    },
    imp_factura(item) {
      console.log(item);
      ...
      let prefijo = "FEVT";
      let consecutivo = parseFloat(item.factura_rep);
      let fecha = item.fecha_rep.substr(0, 4);
      ...
      post
        ...
        .catch((err) => {
          console.log(err);
          this.send_error("Error al imprimir Factura");
        });
    },
    consultar_reporte() {
      if (!this.form.contrato_select) {
        ...
        this.postData({
          url: "dispensa/dlls/RpDispFact01J.dll",
          data: data.join("|") + "|",
          method: "",
        })
          .then((data) => {
