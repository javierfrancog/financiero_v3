        ...
        <div class="pa-0 px-8">
          <v-card-text class="px-0 pa-0">
            <v-row>
              ...
              <v-col class="d-flex" cols="8" sm="6">
                <v-autocomplete
                  ...
                  @change="get_consultar(), errores.asesor = false"
                ></v-autocomplete>
              </v-col>
            ...
            <v-row>
              ...
              <v-col cols="6">
                <canvas id="grafico_cartera" width="100" height="50"></canvas>
              </v-col>
              <v-col cols="6">
                <canvas id="grafico_clientes" width="100" height="50"></canvas>
              </v-col>
            </v-row>
          </v-card-text>
        </div>
...
//    this.get_consultar();
  ...
  methods: {
    ...
    get_ventas() {
      this.loader = true;
      let asesor = this.form.asesor;
      var fecha = this.periodo_cargue.replace(/\-/g, "");
      post
        .postData({
          url: "Financiero/dlls/RpTabCom01J.dll",
          data: sessionStorage.Sesion + "|" + fecha + "|" + asesor + "|",
          method: "",
        })
        .then((res) => {
          this.loader = false;
          ...
          let data = [];
          res.forEach((el) => {
            let valor = el.valor_rep.replace(/\,/g, "");
            let periodo = el.perido_rep;
    ...
    get_clientes() {
      ...
      post
        .postData({
          url: "Financiero/dlls/RpTabCom02J.dll",
          data: sessionStorage.Sesion + "|" + fecha + "|" + asesor + "|",
          method: "",
        })
        .then((res) => {
          ...
          res = res.filter((el) => el.perido_rut && el.perido_rut.trim());
          let labels = [];
          let data = [];
          res.forEach((el) => {
            let valor = el.valor_rut.replace(/\,/g, "");
            let periodo = el.perido_rut;

            data.push(parseFloat(valor));
            labels.push(periodo);
          });

          this.graficar("grafico_clientes", labels, data, "bar", "Clientes Nuevos");
          this.get_cartera();
        })
        .catch((err) => {
          console.log(err)
    ...
    get_cartera() {
      ...
      post
        .postData({
          url: "Financiero/dlls/RpTabCarteraJ.dll",
          data: sessionStorage.Sesion +  "|" + fecha + "|" + asesor + "|",
          method: "",
        })
        .then((res) => {
          this.loader = false;
          res = res.filter((el) => el.perido_car.trim());
          let labels = [];
          let data = [];
          res.forEach((el) => {
            let valor = el.valor_car.replace(/\,/g, "");
            let periodo = el.perido_car;

            data.push(parseFloat(valor));
            labels.push(periodo);
          });

          this.graficar("grafico_cartera", labels, data, "bar", "Cartera (Millones de $)");
        })
        .catch((err) => {
          this.loader = false;
          this.$emit("snack", {
