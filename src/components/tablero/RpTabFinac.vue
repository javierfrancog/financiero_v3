    ...
    <v-flex xs12 md12>
      <v-card class="mx-auto col-12" max-width="1300" elevation="2">
        <v-card-title>
          <v-list-item>
            <v-list-item-icon>
              <v-icon size="30" color="blue darken-4 ">mdi-account-cash-outline</v-icon>
            </v-list-item-icon>
            <v-list-item-content>
              <v-list-item-title class="headline ml-2"
                >Tablero Financiero</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
...
export default {
  ...
  methods: {
    ...
    get_ventas() {
      ...
      post
        .postData({
          url: "Financiero/dlls/RpPrevtas01J.dll",
          data: sessionStorage.Sesion + "|" + fecha + "|",
          method: "",
        })
        .then((res) => {
          this.loader = false;
          res = res.filter((el) => el.tipo_rep.trim());
          let labels = [];
          let data = [];
          res.forEach((el) => {
            let valor = el.valor_rep.replace(/\,/g, "");
            let tipo = el.tipo_rep;

            data.push(parseFloat(valor));
            labels.push(tipo);
          });

          this.graficar("grafico_ventas", labels, data, "bar", "Estado de Resultados (Millones de $)");
        })
        .catch((err) => {
          console.log(err);
          this.loader = false;
