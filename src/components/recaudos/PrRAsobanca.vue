    ...
    <v-flex xs12 md12>
      <v-card class="mx-auto col-12" max-width="1300" elevation="2">
        <v-card-title>
          <v-list-item>
            <v-list-item-icon>
              <v-icon size="30" color="blue darken-4 ">mdi-transfer-up</v-icon>
            </v-list-item-icon>
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Remarcar Asobancaria</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
            ...
            >
              <template v-slot:item.edit="{ item }">
                <v-tooltip top>
                  ...
                  <span>{{ "Remarcar Asobancaria" }}</span>
                </v-tooltip>
              </template>
            </v-data-table>
          </v-card-text>
...
export default {
  ...
  data() {
    return {
      ...
      form: {},
      headersresumen: [
        { text: "Hora", align: "left", value: "hora_rep" },
  ...
  methods: {
    ...
    format_consulta(data) {
      ...
      let datos = {
        url: "Recaudos/dlls/PrRAsobancaJ.dll",
        data:
    ...
    remarcar_asobanca: function (item) {
      var data = JSON.parse(JSON.stringify(item));
      if (!this.form.fecha) {
        this.errores.fecha = true;
        this.send_error("Fecha Obligatoria!");
      } else {
        this.format_remarca(data);
      }
    },
    format_remarca(data) {
      this.card_estado = true;
      var fecha = this.form.fecha.replace(/\-/g, "");
      var convenio = this.form.convenio.cod_rep;
      var nro = this.form.hora;
      var hora = data.hora_rep.replace(/\:/g, "");

      let datos = {
        url: "Recaudos/dlls/PrRAsobanca.dll",
        data:
          sessionStorage.Sesion +
          "|" +
          fecha +
