          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Reporte Por Cajero</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Punto", align: "left", value: "agencia_rep" },
  ...
  methods: {
    ...
    print_reporte_excel() {
      ...
      var header_format = [
        { text: "RECAUDO POR CAJERO", bold: true, size: 16 },
      ...
      this.pdfs.getBase64(logo_src).then((logo) => {
        this.excel
          ._informe({
            ...
            archivo: `RECAUDO POR CAJERO-${new Date().getTime()}`,
          })
          .then((data) => {
    ...
    print_reporte() {
      ...
      data_parse.unshift([
        { text: "Cajero", style: "header_table" },
    ...
    consultar_reporte() {
      ...
      this.postData({
        url: "Recaudos/dlls/RpRecxcajero01J.dll",
        data: data.join("|") + "|",
        method: "",
      })
        .then((data) => {
          ...
          data.forEach((el, index) => {
            ...
            let cajero = el.cajero_rep.trim();
            if (cajero == cajero_anterior) {
              cantidad_subtotal += cantidad;
              ...
              if (index > 0) {
                data_parse.push({
                  id: 99,
                  cajero_rep: "",
                  cantidad_rep: this.formatNumero(cantidad_subtotal),
                  valor_rep: this.formatNumero(valor_subtotal),
          ...
          data_parse.pop();
          this.contenido = JSON.parse(JSON.stringify(data_parse));
        })
        .catch((err) => {
          this.loader = false;
