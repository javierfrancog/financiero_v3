  ...
  <v-layout wrap justify-center class="pa-6">
        ...
        <v-card-text class="pb-0">
          <v-form>
            <div class="pa-2 px-0">
              ...
              <v-row>
                <v-col class="d-flex align-center mx-auto pb-0" cols="6">
                  <v-text-field
                    label="Documento"
                    outlined
    ...
    <v-dialog v-model="dialogo_recaudo.estado" persistent max-width="80%">
      <v-card class="pa-2">
          ...
          <v-row>
            ...
            <v-col class="d-flex" cols="12" sm="2">
              <v-text-field
                ...
                :value="formatNumero(form.recibido)"
                outlined
                hide-details
                type="text"
                disabled
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn class color="red" text @click="dialogo_recaudo.estado = false"
            >Cancelar</v-btn
          >
          <v-btn
            class="white--text"
...
export default {
  ...
  methods: {
    ...
    guardar_tabla() {
      const _this = this;
      let recibido = this.form.recibido;

      if (recibido < 0) {
        this.send_error("El valor recibido debe ser mayor a 0");
      } else if (!_this.comprobanteEditar.cuentaPago) {
        this.send_error("Debe Seleccionar cuenta de pago");
      } else {
        ...
        let recibido = parseFloat(this.form.recibido) || 0;
        ...
        detalle.forEach((item) => {
          ...
          let obj = {
            ...
            creditos,
            debitos: 0,
            fvence: item.fechavence_cart? item.fechavence_cart :this.form.fecha_doc,            
            descripcionCuenta: cuenta_cartera
              ? cuenta_cartera.descrip_puc.trim()
              : "",
            detalle: detalle_recaudo || "",
    ...
    calcular_diferencia() {
      ...
      let rtefte = this.form.rtefte || 0;
      let rteica = this.form.rteica || 0;
      ...
      this.form.recibido = total.toString();
    },
    ...
    seleccionar_item_cancelacion(item) {
      ...
      if (!existe) this.dialogo_cxp.tabla_seleccionados.push(copia);
    },
    seleccionar_item(item) {
      ...
      if (!existe) this.dialogo_recaudo.tabla_seleccionados.push(copia);
    },
    cerrar_registro() {
      let debitos = this.detalleTotales.debitos;
      let creditos = this.detalleTotales.creditos;
    ...
    grabar_registro() {
      ...
      if (!data.tipoDocumento) {
        ...
        var detalle_format = detalle_filtro.map((el) => {
          ...
          el.nrocredito = el.nrocredito ? parseFloat(el.nrocredito) : 0;
    ...
    validarFacturas() {
      ...
      if (item) {
        ...
        if (carteraEmpresa == item.cod_puc.trim())
    ...
    get_facturas_csv() {
      ...
      return new Promise((resolve, reject) => {
        ...
        var sesion = sessionStorage.Sesion;
        let calldll = "";
        if (this.empresa[0].contarem_empr == "1") {
          calldll = "Financiero/dlls/PrCartRemJ.DLL";
        } else {
          calldll = "Financiero/dlls/PrCarteraJ.DLL";
        }
    ...
    get_facturas2() {
      var cuenta = this.comprobanteEditar.cuentaContable;
      var ctacartera_empr = this.empresa[0].ctacartera_empr;

      if (cuenta.cod_puc.trim() == ctacartera_empr) {
    ...
    cargarTabladocc: function () {
      let punto = this.empresa[0].ptoperfil_empr;
      post
        .postData({
          url: "Financiero/dlls/CfDoccontabJ.dll",
          data: sessionStorage.Sesion + "|" + "1" + "|" + punto + "|",
          method: "",
        })
        .then((data) => {
          this.docc = data;
    ...
    cargarTablactas: function () {
      post
        ...
        .then((data) => {
          let bancosperfil_empr = this.empresa[0].bancosperfil_empr;
          let cajaperfil_empr = this.empresa[0].cajaperfil_empr;

          let items = data.filter((el) => {
            return (
              el.cod_puc == bancosperfil_empr || el.cod_puc == cajaperfil_empr
            );
          });

          this.cuentasContables = items;
        })
        .catch((err) => {
    ...
    cargarTablaconceptos: function () {
      let punto = this.empresa[0].ptoperfil_empr;
      let tipo = this.form.tipoDocumento.codigo_docc;
      post
        .postData({
          url: "Financiero/dlls/CfconceptosJ.dll",
          data:
            sessionStorage.Sesion + "|" + 0 + "|" + punto + "|" + tipo + "|",
          method: "",
        })
        .then((data) => {
