<template>
  <v-layout wrap justify-center class="pa-6" id="prfact01">
          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Facturación Comercial</v-list-item-title
              >
            </v-list-item-content>
            <v-card-actions>
              <!-- <v-btn
        ...
        <div class="pa-0 px-8">
          <v-card-text class="px-0 pa-0">
            <v-row>
              ...
              <v-col class="d-flex" cols="3" v-if="tipo_pos != 0">
                <v-autocomplete
                  label="Maquina Pos"
                  v-model="form.maquinapos"
                  :items="maquina"
                  :item-text="format_maquina"
                  item-value="prefijo_resd"
                  hide-details
                  clearable
                  outlined
                  return-object
                  @change="errores.maquina = false"
              ...
              <v-col cols="3" v-if="form.clientes">
                <v-select
                  :items="[
                    ...
                    { text: 'Orden de Compra', value: 5 },
                    { text: 'Proforma', value: 4 },
                  ]"
            ...
            <v-row>
              ...
              <v-col class="d-flex" cols="2" v-if="form.moneda == 2">
                <v-text-field
            ...
            <v-row>
              ...
              <v-col class="d-flex" cols="3">
                <v-switch
                  v-model="form.notadev"
                  @change="select_devol()"
                  label="Nota Devolucion"
                ></v-switch>
              </v-col>
            </v-row>
            <v-row>
            ...
            <v-row sm="6" no-gutters v-if="empresa.presentprod_empr == 1">
              <v-col>
                <v-autocomplete
                  ...
                  @change="errores.presentacion = false"
                  hide-details
    ...
    <v-dialog v-model="dialogo_proforma.proforma" max-width="800" persistent>
      <v-card class="text--white pa-2">
        <v-card-title>Seleccionar Proforma</v-card-title>
        <v-form ref="formArticulo">
          <div class="pa-4 px-8">
            <v-row sm="12" no-gutters>
              <v-col class="mb-0" cols="6">
                <v-autocomplete
                  label="Proforma"
                  :items="lista_proforma"
                  outlined
                  clearable
                  return-object
                  hint="Buscar Proforma"
                  persistent-hint
                  required
                  item-value="nro_pes"
                  v-model="articulo_select.producto"
                  :item-text="format_proforma"
                  :loading="load.servicios"
                >
                </v-autocomplete>
              </v-col>
              <v-col class="mb-0" cols="3">
                <v-btn
                  ...
                  class
                  block
                  @click="dialogo_proforma.proforma = false"
                  >Cancelar</v-btn
                >
              </v-col>
              <v-col class="mb-4" cols="3">
...
export default {
  ...
  methods: {
      ...
      if (pedido) {
        ...
        post
          ...
          .then((res) => {
            ...
            lista.forEach((el) => {
              let cantidad = parseFloat(el.cantidad.replace(/\,/g, ""));
    ...
    select_devol() {
      let seleccion = this.form.notadev ? 1 : 0;
      if (seleccion == 1) {
        this.form.observaciones = "No se aceptan cambios ni devoluciones";
      } else {
    ...
    validar_docOrigen() {
      ...
      if (docOrigen == 1) {
        ...
        post
          .postData({
            url: "ptovta/dlls/PrProforma02J.dll",
            data: datosEnvio,
            method: "",
          })
          .then((res) => {
            _this.lista_proforma = res;
            this.dialogo_proforma.proforma = true;
            this.disabled.grabar_factura = false;
          });
      }
    },
    ...
    agregar_prodproforma() {
      this.dialogo_proforma.proforma = false;
      let nrofact = parseFloat(this.articulo_select.producto.numero_fact);
      let fecha_fact = this.articulo_select.producto.fecha_fact;
      fecha_fact = fecha_fact.substring(0, 4);
      this.get_proforma(nrofact, fecha_fact);
    },
    get_proforma(nrofact, fecha) {
      var agencia = "0001";
      let prefijo = "0001";
      this.proforma = 1;
      ...
      if (nrofact) {
        ...
        post
          ...
          .then((res) => {
            var lista = res[0].productos;
            var nuevo_detalle = [];
            let nro_proforma = res[0].numero_fact.trim();
            lista.forEach((el) => {
              let cantidad = parseFloat(el.cantidad.replace(/\,/g, "")) || 0;
              let iva = parseFloat(el.iva) || 0;
              let ivaTotal = parseFloat(el.vlr_iva.replace(/\,/g, "")) || 0;
              let subtotal =
                parseFloat(el.vlr_subtotal.replace(/\,/g, "")) || 0;
              let unitario = subtotal / cantidad;
              let producto = _this.productos.find(
              ...
              let totalDescuento = 0;
              let centrocto = [];
              centrocto.llavecentro_cost = el.ccosto;
              ...
              nuevo_detalle.push({
                ...
                lista: el.nombre_servicio,
                manual: false,
                observacion: el.detprod,
                producto,
                factorImpoc: 1,
                subtotal,
                ccosto: centrocto,
                umedida: el.codumedida,
                total: _this.formatNumero(total),
                totalDescuento,
                ubicacion,
                ubicaciones: [],
                unidadmed: el.codumedida,
                valor: _this.formatNumero(unitario),
                valorDescuento: descuento,
                valorIva,
              });
    ...
    format_proforma(item) {
      return item.fecha_fact + " - " + parseInt(item.numero_fact);
    },
    async get_enviardian() {
      ...
      post
        ...
        .then((data) => {
          ...
          if (this.empresa.tipoconsorc_empr == 9) {
            textoSocios = "";
          }
          ...
          factura.notes = factura.Observaciones64
            ...
            : textoSocios;
          console.log("titan xxx", factura.notes);
    ...
    grabar_mail(factura, correo, correo2, correo3) {
      ...
      var agencia = parseFloat(factura[0]);
      var prefijo = factura[1].trim();
      var consecutivo = parseFloat(factura[2]);
      var datosEnvio =
    ...
    validar_cliente() {
      ...
      this.form.fpago = parseFloat(this.form.clientes.fpago_rut);
    },
    verificar_codigo() {
      ...
      post
        .postData({
          url: "Ptovta/dlls/CfProductosJB.dll",
          data:
    ...
    async grabar_factura() {
      var data = this.form;
      let pos = data.pref_fact.pos_resd;
      ...
      if (!data.punto_fact) {
      ...
      } else if (!data.maquinapos && this.tipo_pos == 1) {
        this.errores.maquinapos = true;
        this.send_error("Selecciona una maquina Pos");
      } else if (data.ocompra && !data.fechaorden) {
        this.errores.ocompra = true;
        this.send_error("Digite Fecha Orden de Compra");
      } else {
        ...
        this.get_plano(detalle)
          .then((plano) => {
            ...
            let maquinapos = data.maquinapos ? data.maquinapos.cod_rep : "0";
            let ocompra = data.ocompra ? data.ocompra : "0";
            let porc_reteica = data.porcrteica ? data.porcrteica : "0";
            let porc_reteavisos = "0";
            let anticipos = data.anticipos
              ? data.anticipos.replace(/\,/g, "")
              : "0";
            let fechaorden = data.fechaorden
              ? data.fechaorden.replace(/\-/g, "")
              : "0";

            var tipoper = data.pref_fact.tipoper_resd;
    ...
    async print_item(item, getBase64) {
      ...
      var tipoimpresion = this.form.pref_fact.pos_resd;
      if (this.proceso_imp == 1 && this.empresa.id_empr == "000000017311064") {
        tipoimpresion = 1;
        this.proceso_imp = 0;
      }
      var key = agencia + "|" + prefijo + "|" + nrofact + "|" + fecha + "|";
        ...
        this.leer_producto()
          .then(() => {
            ...
            if (producto) {
              ...
              if (this.articulo_select.iva == 99) {
                this.articulo_select.iva = 0;
              }
              var subtotalProducto = 0;
              var ivaProducto = 0;
              var valorProducto = 0;
    ...
    get_listas(tipo) {
      ...
      if (this.form.clientes || tipo == 1) {
        ...
        post
          .postData({
            url: "Financiero/dlls/CflistasLiteJ.dll",
    ...
    cargar_empresa() {
      post
        ...
        .then((data) => {
          ...
          if (parseFloat(this.empresa.id_empr) == 1122141822) {
            this.bloqueo_fecha = true;
          }
        })
    ...
    get_maquinas(prefijo) {
      post
        .postData({
          url: "financiero/dlls//CfCajaspos01J.dll",
          data: sessionStorage.Sesion + "|" + prefijo + "|",
          method: "POST",
        })
        .then((data) => {
          this.maquina = data;
    ...
    get_notifica() {
      post
        ...
        .then((data) => {
          const send_mensaje =
            data[0].mensaje1_rep.trim() + " " + data[0].mensaje2_rep.trim();
    ...
    get_consecutivo() {
      ...
      post
        .postData({
          url: "Ptovta/dlls/PrFacturaN.dll",
          data:
            sessionStorage.Sesion +
            "|" +
        ...
        .then((data) => {
          ...
          const fechaInicio = new Date(this.form.pref_fact.fechafin_resd);
          const fechaFin = new Date(this.form.fecha);
          const diferenciaMs = fechaInicio - fechaFin;
          const dias = diferenciaMs / (1000 * 60 * 60 * 24);

          if (dias < 11) {
            const send_mensaje = "La Resolución se vence en " + dias + " días";
          ...
          let nrofinres = parseFloat(this.form.pref_fact.nrofin_resd);
          if (nrofinres < 10) {
            const send_mensaje =
              "Quedan " +
              nrofinres +
              " facturas disponibles autorizadas en la Resolución !  ";
    ...
    edit_item(item) {
      ...
      let busqueda = this.tablaccosto.find(
        (el) => el.llavecentro_cost == item.ccosto.llavecentro_cost
      );
      this.articulo_select.ccosto = busqueda;

      busqueda = this.unidad_med.find((el) => el.value == item.umedida);
      this.articulo_select.umedida = busqueda;
    },
