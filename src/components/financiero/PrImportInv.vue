          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline ml-2"
                >Importar Inventarios</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
        <div class="pa-0 px-8">
          <v-card-text class="px-0 pa-0">
            <v-row>
              ...
              <v-col sm="3" cols="12">
                <v-select
                  :items="[{ text: 'Tiquetes_Prosoft', value: 1 }]"
                  label="Origen"
                  outlined
                  hide-details
                  v-model="form.origen"
...
export default {
  ...
  methods: {
    ...
    procesar_plano(nombre_adjunto) {
      console.log(nombre_adjunto)
      let fecha = this.form.fecha.replace(/\-/g, "");      
      post
        .postData({
          url: "Molinos/dlls/PrImportbas.dll",
          data: sessionStorage.Sesion + "|" + nombre_adjunto + "|" + fecha + "|",
          method: "",
        })
        .then((data) => {
          this.loaderCard = false
                this.$emit("snack", {
                  color: "success",
                  text: "Inventario cargado correctamente",
                  estado: true,
                })          
        })
        .catch((err) => {
