          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Ventas según Tarifa Iva</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Factura", align: "center", value: "numero_rep" },
        { text: "Subtotal", align: "center", value: "base_rep" },
        { text: "Tarifa-0%", align: "center", value: "tar0_rep" },
        { text: "Tarifa-5%", align: "center", value: "tar5_rep" },
        { text: "Tarifa-8%", align: "center", value: "tar8_rep" },
        { text: "Tarifa-19%", align: "center", value: "tar19_rep" },
        { text: "Otras Tarifas", align: "center", value: "tarotros_rep" },
        { text: "Descuentos", align: "center", value: "dcto_rep" },
        { text: "Total", align: "center", value: "total_rep" },
        { text: "IdDian", align: "center", value: "iddian_rep" },
      ],
      errores: {
        codigo: false,
        estado: false,
  ...
  methods: {
    generar_excel() {
      ...
      data.forEach((el) => {
        data_parse.push({
          tipodoc_rep: el.tipodoc_rep.trim(),
          fecha_rep: el.fecha_rep,
          agencia_rep: el.agencia_rep.trim(),
          pref_rep: el.pref_rep === "" ? "" : el.pref_rep.trim(),

          numero_rep:
            el.numero_rep === ""
              ? 0
              : parseFloat(el.numero_rep.replace(/\ /g, "").replace(/\,/g, "")),
          base_rep:
            el.base_rep === ""
              ? 0
              : parseFloat(el.base_rep.replace(/\ /g, "").replace(/\,/g, "")),
          tar0_rep:
            el.tar0_rep === ""
              ? 0
              : parseFloat(el.tar0_rep.replace(/\ /g, "").replace(/\,/g, "")),
          tar5_rep:
            el.tar5_rep === ""
              ? 0
              : parseFloat(el.tar5_rep.replace(/\ /g, "").replace(/\,/g, "")),
          tar8_rep:
            el.tar8_rep === ""
              ? 0
              : parseFloat(el.tar8_rep.replace(/\ /g, "").replace(/\,/g, "")),

          tar19_rep:
            el.tar19_rep === ""
              ? 0
              : parseFloat(el.tar19_rep.replace(/\ /g, "").replace(/\,/g, "")),
          tarotros_rep:
            el.tarotros_rep === ""
              ? 0
              : parseFloat(
                  el.tarotros_rep.replace(/\ /g, "").replace(/\,/g, "")
                ),
          dcto_rep:
            el.dcto_rep === ""
              ? 0
      ...
      var header = [
        ...
        "Ventas según Tarifa".toUpperCase(),
      ...
      this.pdfs.getBase64(logo_src).then((logo) => {
        this.excel
          ._informe({
            ...
            archivo: `Ventas-Tar-${new Date().getTime()}`,
          })
          .then((data) => {
    ...
    generar_pdf() {
      ...
      contenido.forEach((el) => {
        subtotal += parseFloat(
          el.base_rep.replace(/\ /g, "").replace(/\,/g, "")
        );
        tarifa_0 += parseFloat(
          el.tar0_rep.replace(/\ /g, "").replace(/\,/g, "")
        );
        tarifa_5 += parseFloat(
          el.tar5_rep.replace(/\ /g, "").replace(/\,/g, "")
        );
        tarifa_19 += parseFloat(
          el.tar19_rep.replace(/\ /g, "").replace(/\,/g, "")
        );
        otras += parseFloat(
          el.tarotros_rep.replace(/\ /g, "").replace(/\,/g, "")
        );
        descuentos += parseFloat(
          el.dcto_rep.replace(/\ /g, "").replace(/\,/g, "")
        );

        total += parseFloat(el.total_rep.replace(/\ /g, "").replace(/\,/g, ""));
        ...
        datosTabla.push([
          ...
          },
          {
            fontSize: 8,
            alignment: "left",
            text: el.numero_rep.trim(),
          },

          {
            fontSize: 8,
            alignment: "right",
            text: el.base_rep.replace(/\ /g, ""),
          },
          {
            fontSize: 8,
            alignment: "right",
            text: el.tar0_rep.replace(/\ /g, ""),
          },
          {
            fontSize: 8,
            alignment: "right",
            text: el.tar5_rep.replace(/\ /g, ""),
          },
          {
            fontSize: 8,
            alignment: "right",
            text: el.tar19_rep.replace(/\ /g, ""),
          },
          {
            fontSize: 8,
            alignment: "right",
            text: el.tarotros_rep.replace(/\ /g, ""),
          },
      ...
      datosTabla.push([
...

        {
          fontSize: 8,
          alignment: "right",
          text: this.formatNumero(subtotal),
        },
        {
          fontSize: 8,
          alignment: "right",
          text: this.formatNumero(tarifa_0),
        },
        {
          fontSize: 8,
          alignment: "right",
          text: this.formatNumero(tarifa_5),
        },
        {
          fontSize: 8,
          alignment: "right",
          text: this.formatNumero(tarifa_19),
        },
        {
          fontSize: 8,
          alignment: "right",
          text: this.formatNumero(otras),
        },
        {
          fontSize: 8,
          alignment: "right",
          text: this.formatNumero(descuentos),
        },
        {
          fontSize: 8,
          alignment: "right",
          text: this.formatNumero(total),
        },
      ]);
    ...
    cargarcontenido() {
      ...
      post
        .postData({
          url: "ptovta/dlls/RpVtasTarJ.dll",
          data:
