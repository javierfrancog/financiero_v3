    ...
    <v-flex xs12 md12>
      <v-card class="mx-auto col-12" max-width="1300" elevation="2">
        <v-card-title>
          <v-list-item>
            <v-list-item-icon>
              <v-icon size="30" color="blue darken-4 ">mdi-ballot-recount</v-icon>
            </v-list-item-icon>
            <v-list-item-content>
              <v-list-item-title class="headline ml-2">Reprocesar Facturas</v-list-item-title>
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
        <div class="pa-0 px-8">
...
export default {
  ...
  methods: {
    ...
    get_comprobantes() {
      ...
      var docconta = data.pref_fact.doconta_resd;
      ...
      post
        .postData({
          url: "Ptovta/dlls/PrFact01R.DLL",
          data: sessionStorage.Sesion + "|" + fecha + "|" + agencia + "|" + docconta + "|"+ tipoper + "|" + prefijo + "|",
          method: ""
        })
        .then(data => {
          this.loader = false;
