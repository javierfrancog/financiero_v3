          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Comision por Ventas</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Numero", align: "center", value: "factura_rep" },
        { text: "Cliente", align: "left", value: "cliente_rep" },
        { text: "Vlr_Venta", align: "center", value: "vlrvta_rep" },
        { text: "% Comision", align: "center", value: "porc_rep" },
        { text: "Vlr_Comisión", align: "center", value: "comision_rep" },
      ],
      errores: {
        codigo: false,
        asesor: false,
  ...
  methods: {
    generar_excel() {
      ...
      data.forEach((el) => {
        data_parse.push({
          ...
          factura_rep: el.factura_rep === "" ? 0 : parseFloat(el.factura_rep.replace(/\ /g, "").replace(/\,/g, "")),

          cliente_rep: el.cliente_rep === "" ? "" : el.cliente_rep.trim(),
          vlrvta_rep: el.vlrvta_rep === "" ? 0 : parseFloat(el.vlrvta_rep.replace(/\ /g, "").replace(/\,/g, "")),
          porc_rep:
            el.porc_rep === ""
              ? 0
              : parseFloat(el.porc_rep.replace(/\ /g, "").replace(/\,/g, "")),
          comision_rep: el.factura_rep === "" ? 0 : parseFloat(el.comision_rep.replace(/\ /g, "").replace(/\,/g, "")),
       });
      });
      ...
      var header = [
        ...
        "Comisiones por Ventas".toUpperCase(),
      ...
      this.pdfs.getBase64(logo_src).then((logo) => {
        this.excel
          ._informe({
            sheetName: "Comisiones por Ventas",
            header: header,
            logo,
            tabla: {
              columnas: columnas,
            ...
            archivo: `Com-Ventas-${new Date().getTime()}`,
          })
          .then((data) => {
    ...
    generar_pdf() {
      ...
      contenido.forEach((el) => {
        totalContado += parseFloat(
          el.contado_rep.replace(/\ /g, "").replace(/\,/g, "")
        );
        totalCredito += parseFloat(
          el.credito_rep.replace(/\ /g, "").replace(/\,/g, "")
        );
        totalIVA += parseFloat(
          el.iva_rep.replace(/\ /g, "").replace(/\,/g, "")
        );
        totalDctos += parseFloat(
          el.dcto_rep.replace(/\ /g, "").replace(/\,/g, "")
        );
        totalT += parseFloat(
          el.total_rep.replace(/\ /g, "").replace(/\,/g, "")
        );
