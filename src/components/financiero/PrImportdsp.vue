          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline ml-2"
                >Importar Documento Soporte</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
            ...
            <v-row>
              ...
              <v-col class="d-flex" cols="3">
                <v-autocomplete
                  ...
                  outlined
                  return-object
                  @change="get_resolucion();  errores.agencia = false"
              ...
              <v-col  cols="3">
                <v-select
                  :items="[
                    { text: 'Prosoft_V.Nueva', value: 1 },
                    { text: 'Prosoft_V.RM', value: 2 },
                  ]"
                  label="Origen"
                  outlined
                  hide-details
  ...
  methods: {
    ...
    subir_archivo() {
      ...
      if (adjunto) {
        ...
        var nombre_adjunto = `${sesion}_DSP`;
    ...
    get_resolucion() {
      var prefijo = this.form.agencia.codigo_agc;
      post
        .postData({
          url: "financiero/dlls/CfResdianJ.dll",
          data: sessionStorage.Sesion + "|" + prefijo + "|" + "1" + "|",
          method: "POST",
        })
        .then((data) => {
          data.pop();
        ...
        .catch((err) => {
          $this.$emit("snack", {
            color: "error",
            text: "Error al cargar Resoluciones: " + err.Mensaje[0].Error,
            estado: true,
          });
        });
    },    
    ...
    procesar_plano(nombre_adjunto) {
      ...
      let dll = "0";
      if (origen === 1) dll = "Financiero/dlls/PrDspImp1.dll";
      else if (origen === 2) dll = "Financiero/dlls/PrDspImp1.dll";
      post
        .postData({
          url: dll,
          data:
        ...
        .then((data) => {
          ...
          this.$emit("snack", {
            color: "success",
            text: "Documento Soporte cargado correctamente",
            estado: true,
          });
        })
        .catch((err) => {
          ...
          this.$emit("snack", {
            color: "error",
            text: "Error al procesar Importación",
            estado: true,
          });
        });
    },
