          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Reporte Recaudos Cartera</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
...
export default {
  ...
  methods: {
    ...
    descargar_excel() {
      let data = this.contenido;
      let data_parse = [];
      let fecha_corte = this.periodo_cargue;

      data.forEach((el, i) => {
        let copia = JSON.parse(JSON.stringify(el));
        copia.pagos_rep = parseFloat(el.pagos_rep.replace(/\,/g, "")) || 0;
      ...
      let header_format = [
        { text: "RECAUDOS DE CARTERA", bold: true, size: 16 },
      ...
      this.pdfs.getBase64(logo_src).then((logo) => {
        this.excel
          ._informe({
            ...
            archivo: `RECAUDOS CARTERA-${new Date().getTime()}`,
          })
          .then((data) => {
    ...
    cargarcontenido() {
      var data = this.form;
        ...
        let calldll = calldll = "Ptovta/dlls/RprecCartJ.DLL";
        var datosEnvio =
          sesion +
          "|" +
          punto +
