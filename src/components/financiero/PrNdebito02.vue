          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Reimpresión Nota Débito</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
...
import { ndebito_titan } from "../../_formatos.pdf";
...
export default {
  ...
  methods: {
    ...
    elimina_item(item) {
      ...
      post
        .postData({
          url: "Ptovta/dlls/PrNdebitoX.dll",
          data: datosEnvio,
          method: "",
        })
        .then((data) => {
        ...
        .catch((err) => {
          ...
          this.card.disabled = false;
          this.send_error(
            "Ha ocurrido un error eliminando la Nota"
          );
        });
    },
    async get_pdf_titan(item, getBase64) {
      ...
      return await post
        .postData({
          url: "Ptovta/dlls/PrNdebito01J.dll",
          data: datosEnvio,
          method: "",
        })
        .then(async (data) => {
    ...
    async generar_pdf_2(data, getBase64) {
      ...
      if (getBase64) return await ndebito_titan(data, getBase64);
      else ndebito_titan(data);
    },
    get_json_dian(item, referencia) {
      ...
      post
        ...
        .then((data) => {
          ...
          detalles = factura.debit_note_lines.map((item) => {
    ...
    cargarcontenido() {
      ...
      if (!agencia) {
        ...
        post
          .postData({
            url: "ptovta/dlls/PrNDebito02J.dll",
            data:
              sessionStorage.Sesion +
              "|" +
              agencia.codigo_agc +
