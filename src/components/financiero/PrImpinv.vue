...
export default {
  ...
  methods: {
    print_doc(item) {
      ...
      let datosEnvio = sessionStorage.Sesion + "|" + tipoDocumento + "|" + fecha + "|" + nro + "|" + agencia + "|" + 0 + "|";
      post
        .postData({
    ...
    generar_pdf(data) {
      data.registros.pop()
       data.registros.map((item) => {
                ...
                if (nombre64) item.producto = nombre64;
                return item;
            });
      ...
      this.pdfs.getBase64(logo_src).then((logo) => {
        ...
        if (data.nombredoc.trim() == "ENTRADA TRASLADO PRODUCTOS") {
          comprobantes_inv_pos(data).then(() => {
