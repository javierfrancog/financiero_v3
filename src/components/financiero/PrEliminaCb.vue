    ...
    <v-flex xs12 md12>
      <v-card class="mx-auto col-12" max-width="1300" elevation="2">
        <v-card-title>
          <v-list-item>
            <v-list-item-icon>
              <v-icon size="30" color="blue darken-4 ">mdi-alien-outline</v-icon>
            </v-list-item-icon>
            <v-list-item-content>
              <v-list-item-title class="headline ml-2">Elimina Comprobantes Contables</v-list-item-title>
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
        <div class="pa-0 px-8">
...
export default {
  ...
  methods: {
    ...
    borrar_doc(item) {
        var tipoDocumento = this.form.tipoDocumento.agencia_docc +
                            this.form.tipoDocumento.codigo_docc;      
        var fecha = this.periodo_cargue.replace(/\-/g, "");
        var nro = parseInt(item.consecutivo.replace(/\,/g, ""));
      post
        .postData({
          url: "Financiero/dlls/PrEliminaCb.dll",
