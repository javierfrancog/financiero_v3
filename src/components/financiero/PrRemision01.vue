          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Remisión Comercial</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
            ...
            <v-row class="d-flex justify-end">
              <v-col sm="6">
                <v-text-field
                  ...
                  hide-details
                  disabled
                  v-model="articulo_select.facturadas"
                ></v-text-field>
              </v-col>

              <v-col sm="6">
                <v-text-field
                  label="Cantidad a Remisionar"
                  outlined
                  hide-details
  ...
  methods: {
      ...
      if (!data.punto_fact) {
        ...
        this.get_plano(detalle)
          .then((plano) => {
            ...
            post
              .postData({
                url: "Ptovta/dlls/PrRemision01.dll",
                data: datosEnvio,
                method: "",
              })
              .then((data) => {
              ...
              .catch((err) => {
                ...
                this.disabled.btn_enviar = false;
                console.error(err);
                this.send_error("Error al grabar el Remisión");
              });
          })
          .catch((err) => {
            this.disabled.btn_enviar = false;
    ...
    get_plano(detalle) {
      ...
      return new Promise((resolve, reject) => {
        ...
        formData.append("tipo", "Remision01");
      ...
      if (factura) {
        ...
        post
          ...
          .then((data) => {
            ...
            let nombre64 = " ";
            let detalles = this.tablaDetalle.data.map((item) => {
