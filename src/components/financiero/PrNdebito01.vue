    ...
    <v-flex xs12 md12>
      ...
      >
        <v-card-title>
          <v-list-item>
            <v-list-item-icon>
              <v-icon size="30" color="blue darken-4 ">mdi-plus-box</v-icon>
            </v-list-item-icon>
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Nota Débito</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
...
export default {
  ...
  methods: {
    ...
    get_enviardian() {
      ...
      let urlconsulta = "Ptovta/dlls/PrNdebTitanJ.dll";
      ...
      post
        ...
        .then((data) => {
          ...
          let detalles = " ";
          urlEnvio =
            "https://server1ts.net/financiero/inc/api_dian/ndelect.titan.php";
          ...
          fetch(urlEnvio, {
            ...
            .then((response) => {
              ...
              if (cude) {
                ...
                this.card.loader = false;
                this.card.disabled = false;
                this.send_error("Error enviando la Nota Debito electrónica");
              }
            });
        })
        .catch((err) => {
    ...
    grabar_cufe(res) {
      ...
      post
        .postData({
          url: "Ptovta/dlls/PrNdebito01Id.dll",
          data: datosEnvio,
          method: "",
        })
        .then((data) => {
      ...
      if (!data.clientes) {
        ...
        this.get_plano(detalle)
          .then((plano) => {
            ...
            post
              .postData({
                url: "Ptovta/dlls/PrNdebito01.dll",
                data: datosEnvio,
                method: "",
              })
              .then((data) => {
    ...
    get_consecutivo() {
      ...
      post
        .postData({
          url: "Ptovta/dlls/PrNDebitoN.dll",
          data:
            sessionStorage.Sesion +
            "|" +
    ...
    get_info_factura() {
      ...
      if (factura) {
        ...
        post
          .postData({
            url: "Ptovta/dlls/PrNCreditoPJ.dll",
            data: datosEnvio,
            method: "",
          })
          .then((data) => {
            ...
            let nombre64 = " ";
            let detalles = productos.map((item) => {
              nombre64 = window.atob(
                item.base1_pr.trim() +
                  item.base2_pr.trim() +
                  item.base3_pr.trim() +
            ...
            this.tablaDetalle.data = detalles;
          })
          .catch((err) => {
            console.log(err);
    ...
    get_facturas() {
      ...
      if (rut) {
        ...
        post
          .postData({
            url: "Ptovta/dlls/PrCmFacturasJ.dll",
            data: datosEnvio,
            method: "",
          })
          .then((data) => {
