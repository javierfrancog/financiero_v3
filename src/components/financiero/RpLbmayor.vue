  ...
  methods: {
    ...
    get_reporte(tipo) {
      ...
      } else 
      {
        ...
        var datos = {
          url: "Financiero/dlls/RpLbmayorJ.dll",
          data:
    ...
    imprimir_excel() {
      ...
      data.forEach((el) => {
        ...
        copia.ant_excel = parseFloat(el.SDO_ANTER.replace(/\,/g, "")) || 0;
        copia.deb_excel = parseFloat(el.DEBITOS.replace(/\,/g, "")) || 0;
        copia.cre_excel =
          parseFloat(el.CREDITOS.replace(/\,/g, "").trim()) || 0;
        copia.saldo_excel = parseFloat(el.SALDO.replace(/\,/g, "")) || 0;
      ...
      this.pdfs.getBase64(logo_src).then((logo) => {
        this.excel
          ._informe({
            ...
            archivo: `MAYOR Y BALANCES-${new Date().getTime()}`,
          })
          .then((data) => {
