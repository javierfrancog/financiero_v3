    ...
    <v-flex xs12 md12>
      <v-card class="mx-auto col-12" max-width="1300" elevation="2">
        <v-card-title>
          <v-list-item>
            <v-list-item-icon>
              <v-icon size="30" color="blue darken-4 ">mdi-oil</v-icon>
            </v-list-item-icon>
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Orden Combustible</v-list-item-title
              >
            </v-list-item-content>
            <v-row justify="end">
              <v-btn
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Nro Orden", align: "center", value: "nroexterno_rep" },
  ...
  methods: {
    ...
    guardar: function () {
      ...
      } else if (!data.tipocomb) {
        this.errores.tipocomb = true;
        this.send_error("Debe seleccionar registrar un tipo de combustible");
      } else if (!data.valor) {
    ...
    format_envio(data) {
      ...
      let cliente = parseFloat(this.form.orden.id_fact);
      let nroorden = parseFloat(data.orden.numero_fact);
      let nromanual = data.cmanual.trim();
      let tipocomb = data.tipocomb.value;
      let cargue = data.cargue.nro_rep;
      let ordexteno = data.orden.orden_fact;
      let valor = data.valor.replace(/\,/g, "");
      let datos = {
        url: "/Trmcias/dlls/PrOrdencomb.dll",
        data:
    ...
    agregar_item() {
      ...
      this.dialogo.titulo = "Nueva Solicitud";
    ...
    cargarcontenido() {
      ...
      if (this.form.fechaini) {
        ...
        post
          .postData({
            url: "/Trmcias/dlls/PrOrdencomb01J.dll",
