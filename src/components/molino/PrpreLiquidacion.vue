          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Pre_Liquidación Compra Productos Agrícolas</v-list-item-title
              >
            </v-list-item-content>
            ...
            <v-row>
              ...
              <v-col class="d-flex" cols="2">
                ...
                >
                  ...
                  <v-date-picker
                    v-model="form.fechaini"
                    @input="dialogPickerini = false,calcular_final()"
                  ></v-date-picker>
                </v-menu>
              </v-col>
              <v-col class="d-flex" cols="2">
  ...
  methods: {
    ...
    async grabar_documento() {
      ...
      if (!data.punto_fact) {
        ...
        let cartera = "";
        // let get_plano = await this.get_cartera();        
        await this.get_plano(detalle)
          .then((plano) => {
            var sesion = sessionStorage.Sesion;
            var agencia = data.punto_fact.codigo_agc;
            var consecutivo = parseFloat(data.consecutivo);
            ...
            post
              .postData({
                url: "Molinos/dlls/PrpreLiquida.dll",
                data: datosEnvio,
                method: "",
              })
              .then((data) => {
                ...
                this.$emit("snack", {
                  color: "success",
                  text: "Pre Liquidación grabada correctamente",
                  estado: true,
                });
