          ...
          <v-list-item>
            <v-list-item-icon>
              <v-icon size="30" color="blue darken-4 "
                >mdi-abugida-devanagari</v-icon
              >
            </v-list-item-icon>
            <v-list-item-content>
              <v-list-item-title class="headline"
                >ReimpresiÃ³n Pre_Liquidaciones</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
            ...
            >
              ...
              <template v-slot:item.edit="{ item }">
                <v-tooltip top>
                  ...
                  <span>Imprimir Pre_LiquidaciÃ³n</span>
                </v-tooltip>
...
import { preliquidamol } from "../../_formatos_mol.js";
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Agencia", align: "center", value: "agencia_liq" },
        { text: "Fecha", align: "center", value: "fecha_liq" },
        { text: "LiquidaciÃ³n", align: "center", value: "nro_liq" },
        { text: "DescripciÃ³n", align: "left", value: "descriprut_liq" },
  ...
  methods: {
    ...
    print_item(item) {
      ...
      post
        .postData({
          url: "Molinos/dlls/RppreLiquida02J.dll",
          data: datosEnvio,
          method: "",
        })
        .then((data) => {
    ...
    contab_item(item) {
      ...
      var key = agencia + "|" + nro + "|" + fecha + "|";
      var datosEnvio = sessionStorage.Sesion + "|" + key;
      post
        .postData({
          url: "Molinos/dlls/PrContLiq.dll",
          data: datosEnvio,
          method: "",
        })
        .then((data) => {
          ...
          this.card.disabled = false;
            this.$emit("snack", {
              color: "success",
              text: "LiquidaciÃ³n Contabilizada correctamente",
              estado: true,
            });
        ...
        .catch((err) => {
          ...
          this.card.loader = false;
          this.card.disabled = false;
          this.send_error("Error al Recontabilizar el documento");
        });
    },
    dsp_item(item) {
      this.card.loader = true;
    ...
    anular_item(item) {
      ...
      var agencia = item.agencia_liq;
      var nro = item.nro_liq.trim();
      var estado = item.estado_liq.trim();
      if (estado === "1"){
        estado = 0
      } else {
        estado = 1
      }
      var key = agencia + "|" + nro + "|" + estado + "|";

      var datosEnvio = sessionStorage.Sesion + "|" + key;
      post
        .postData({
          url: "Molinos/dlls/PrAnulaLiq.dll",
          data: datosEnvio,
          method: "",
        })
        .then((data) => {
          ...
          this.card.disabled = false;
            this.$emit("snack", {
              color: "success",
              text: "LiquidaciÃ³n Anulada correctamente",
              estado: true,
            });
    ...
    cargarcontenido() {
      ...
      if (!agencia) {
        ...
        post
          .postData({
            url: "Molinos/dlls/RpPreLiquida01J.dll",
            data:
