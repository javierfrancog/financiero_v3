          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Actualiza Liquidación Compra Productos
                Agrícolas</v-list-item-title
              >
            </v-list-item-content>
            ...
            <v-row>
              ...
              <v-col cols="3">
                <v-select
                  ...
                  @change="retencion_change(), (errores.iva = false)"
...
export default {
  ...
  methods: {
    ...
    get_facturas() {
      ...
      var rut = this.form.proveedor.identificacion_rut;
      this.loader = true;
    ...
    retencion_change() {
      ...
      console.log("retencion", this.form.impuesto);
      let impuesto = this.form.impuesto;
      let valor_total = this.tablaDetalle.totales.valor_total;
      valor_total = parseFloat(valor_total.replace(/\,/g, "")) || 0;
      let calculo = 0;
    ...
    iva_change() {
      ...
      let descargue = this.form.descargue
        ? this.form.descargue.replace(/,/g, "")
        : 0;
        let vlrfenalce = this.form.vlrfenalce?
            vlrfenalce = this.form.vlrfenalce : "0"
        
      let fenalce = this.form.fenalce
    ...
    calcular_saldo() {
      ...
      console.log("Calcular_saldo", neto)
    ...
    async grabar_documento() {
      ...
      if (!data.punto_fact) {
      ...
      } else if (!data.impuesto) {
        this.errores.impuesto = true;
        this.send_error("Selecciona tarifa de Retención");
      } else if (!data.iva) {
        ...
        this.get_plano(detalle)
          .then((plano) => {
            ...
            var proveedor = parseFloat(data.proveedor.identificacion_rut);
            ...
            var fomento = data.farrocero
              ...
              : "0";
            var fenalce = data.fenalce
              ? data.vlrfenalce.replace(/,/g, "")
              : "0";
            ...
            post
              .postData({
                url: "Molinos/dlls/PrLiquidaM.dll",
                data: datosEnvio,
                method: "",
              })
              .then((data) => {
    ...
    get_liquidacion() {
      ...
      var nro = this.form.consecutivo.trim();
      var key = agencia + "|" + nro + "|";
      var datosEnvio = sessionStorage.Sesion + "|" + key;
      post
        .postData({
          url: "Molinos/dlls/RpLiquida03J.dll",
          data: datosEnvio,
          method: "",
        })
        .then((data) => {
          this.form.fecha = data[0].fechacorta_liq02;
          var busqueda_proveedor = this.clientes.find(
            (el) => el.identificacion_rut == parseInt(data[0].idrut_liq02)
          );
          this.form.proveedor = busqueda_proveedor;

          let data_parse = JSON.parse(JSON.stringify(data[0].registros));
          data_parse.pop();
          let descripcion64 = " ";
          ...
          data_parse.forEach((item, index) => {
            let obj = {
              idItem: index,
              agencia_pes: item.agencia_liq02,
              nro_pes: item.nropesaje_liq02.trim(),
              producto_pes: item.producto_pes.trim(),
              pesoneto_pes: item.cantidad_liq02.trim(),
              humini_pes: item.humini_liq02.trim(),
              impini_pes: item.impini_liq02.trim(),
              gpart_pes: item.gpart_liq02.trim(),
              rojoini_pes: item.rojoini_liq02.trim(),
              yesoini_pes: item.yesoini_liq02.trim(),
              pesocomp_pes: item.pesocomp_liq02.trim(),
              vlrkilo_pes: item.vlrkilo_liq02.trim(),
              vlrtotal_pes: item.vlrprod_liq02.trim(),
              descmol_pes: item.descargues_liq02.trim(),
              desccanoa_pes: item.descanoa_liq02.trim(),
            };
            this.tablaDetalle.data.push(obj);
          });
          this.form.asociado = parseFloat(data[0].registros[0].asociado_pes);
          let fenalce = parseFloat(
            data[0].fenalce_liq02.trim().replace(/,/g, "")
          );
          if (fenalce > 0) {
            this.form.vlrfenalce = fenalce;
            this.form.fenalce = 1;
          }
          this.form.impuesto = 9;
          this.form.textimpuesto = null;

          let bolsa = parseFloat(data[0].bolsa_liq02.trim().replace(/,/g, ""));
          if (bolsa > 0) {
            this.form.textimpuesto = this.formatNumero(bolsa);
            this.form.impuesto = 1;
          }

          let rtefte = parseFloat(
            data[0].rtefte_liq02.trim().replace(/,/g, "")
          );
          if (rtefte > 0) {
            this.form.textimpuesto = this.formatNumero(rtefte);
            this.form.impuesto = 2;
          }

          let descargue = data[0].descargues_liq02.trim().replace(/,/g, "");
          this.form.descargue = descargue;

          this.form.farrocero = data[0].fomento_liq02.trim();
          this.form.cargue = data[0].cargues_liq02.trim();
          // this.form.impuesto = data[0].bolsa_liq02.trim();

          this.facturasPendientes = [];
          let data_cartera = JSON.parse(JSON.stringify(data[0].descuentos));
          data_cartera.pop();

          let filtro = data_cartera.filter((e) => e.factura_liq02);
          filtro = filtro.map((el) => {
            el.factura = el.pref_liq02.trim() + el.factura_liq02.trim();

            return el;
          });

          data_cartera.forEach((item, index) => {
            let obj = {
              fecha_cart: item.fecha_liq02,
              pref_cart: item.pref_liq02.trim(),
              numero_cart: item.factura_liq02.trim(),
              cuenta_cart: item.codpuc_liq02,
              descrcuenta_cart: item.cuenta_liq02.trim(),
              valor_cart: item.valordesc_liq02.trim(),
            };
            if (item.cuenta_liq02 === "Fenalce") {
              console.log("Fenalce");
            } else if (item.cuenta_liq02 === "Retencion en la Fuente") {
              console.log("rtefte");
            } else if (item.cuenta_liq02 === "Bolsa Nacional Agropecuaria") {
              console.log("bolsa");
            } else if (item.cuenta_liq02 === "Cargues") {
              console.log("Cargues");
            } else if (item.cuenta_liq02 === "Descargues") {
              console.log("Descargues");
            } else if (!item.valordesc_liq02) {
              console.log("vacio");
            } else {
              this.facturasPendientes.push(obj);
            }
            this.calcular_saldo()
          });
        })
