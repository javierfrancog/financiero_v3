          ...
          <v-list-item>
            <v-list-item-icon>
              <v-icon size="30" color="blue darken-4 "
                >mdi-format-wrap-square</v-icon
              >
            </v-list-item-icon>
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Detallado de Cartera</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Valor", align: "center", value: "vlrfact_rep" },
        { text: "Saldo", align: "center", value: "vlrsaldo_rep" },
  ...
  methods: {
    ...
    async print_reporte_excel() {
      ...
      var data_parse = data.map((el) => {
        ...
        el.conceptos.forEach((j) => {
          j.sdofinancia_rep =
            parseFloat(j.sdofinancia_rep.replace(/\,/g, "")) || 0;

          el[j.codigo_rep] = j.sdofinancia_rep;
     ...
     cargos.forEach((c) => {
      ...
      var header_format = [
        { text: "DETALLADO CARTERA", bold: true, size: 16 },
      ...
      this.pdfs.getBase64(logo_src).then((logo) => {
        this.excel
          ._informe({
            ...
            archivo: `DETALLADO CARTERA-${new Date().getTime()}`,
          })
          .then((data) => {
    ...
    cargarfactura(item) {
      ...
      post
        .postData({
          url: "Servdm//dlls/PrCartera2J.dll",
          data:
    ...
    cargarcontenido() {
      ...
      post
        .postData({
          url: "Servdm//dlls/RpCartera2J.dll",
          data:
            sessionStorage.Sesion +
            "|" +
            periodoini +
