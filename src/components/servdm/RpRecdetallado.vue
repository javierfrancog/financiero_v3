          ...
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title class="headline"
                >Reporte Detallado de Recaudos</v-list-item-title
              >
            </v-list-item-content>
          </v-list-item>
        </v-card-title>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Cod_Usuario", align: "center", value: "codusuario_rep" },
  ...
  methods: {
    anularfactura(item) {
      let agencia = item.codagencia_rep;
      let fecha = item.fecha_rep.replace(/\-/g, "");
      let convenio = item.idconvenio_rep;
      let consec = item.factura_rep;
      this.loader = true;
      post
        .postData({
          url: "Servdm//dlls/PrAnulaRec.dll",
          data:
    ...
    print_reporte_excel() {
      ...
      var data_parse = data.map((el) => {
        ...
        el.codusuario_rep = parseFloat(el.codusuario_rep) || 0;
        el.cedula_rep = parseFloat(el.cedula_rep) || 0;
        el.factura_rep = parseFloat(el.factura_rep) || 0;       
    ...
    consultar_reporte() {
      ...
      let pendientes = this.pterad ? 1 : 0;
      let data = [
        sessionStorage.Sesion,
        fechaInicial,
        fechaFinal,
      ...
      this.postData({
        url: "servdm/dlls/RpRecdetalladoJ.dll",
        data: data.join("|") + "|",
        method: "",
      })
        .then((data) => {
