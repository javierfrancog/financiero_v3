    ...
    <v-flex xs12 md12>
      <v-card class="mx-auto col-12" max-width="1300" elevation="2">
        <v-card-title>
          <v-list-item>
            <v-list-item-icon>
              <v-icon size="30" color="blue darken-4 ">mdi-file-document-edit</v-icon>
            </v-list-item-icon>
            <v-list-item-content>
              <v-list-item-title class="headline">Configuración Conceptos</v-list-item-title>
            </v-list-item-content>
            <v-row justify="end">
              <v-btn
                color="success"
...
export default {
  ...
  data() {
    return {
      ...
      form: {},
      tipoconcepto: [
        { text: "1. Ingreso", value: 1 },
        { text: "2. Egreso", value: 2 },
      ],
      headers: [
        { text: "", value: "data-table-expand" },
        { text: "Codigo", align: "center", value: "id_conc" },
        { text: "Tipo", align: "center", value: "tipo_conc" },
  ...
  methods: {
    editar_item(data) {
      ...
      let busqueda_tipo = this.tipoconcepto.find(
        (el) => el.value == item.tipo_conc
      );
      this.form.eps = busqueda_tipo;
      this.form.tipoconcepto = busqueda_tipo;
      this.dialogo.tipo = item.tipo_conc;
      this.form.codigo = item.id_conc;
      this.form.descripcion = item.descrip_conc;
      this.form.ctacontable = item.cuenta_conc.trim();
    },
    guardar: function() {
    ...
    format_envio(data) {
      ...
      let tipo = data.tipoconcepto.value;
      let cuenta = data.ctacontable;
      var eliminar = data.eliminar ? "1" : "0";

      let datos = {
        url: "Clinico/dlls/CfConcepto.dll",
        data:
